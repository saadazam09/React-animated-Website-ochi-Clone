...
function Navbar() {
  ...
  useEffect(() => {
    const sections = ['home', 'services', 'our-work', 'about-us', 'insight', 'contact'];
    ...
    const handleScroll = () => {
      ...
      for (let i = 0; i < sections.length; i++) {
        ...
        if (section && window.scrollY >= section.offsetTop - 60 && window.scrollY < section.offsetTop + section.offsetHeight - 60) {
  ...
  return (
    <div className="fixed z-50 w-full px-5 sm:px-10 md:px-20 py-4 sm:py-6 md:py-8 flex justify-between items-center cursor-pointer bg-gray-900">
      {/* Logo Placeholder */}
...
<path d="M52.4097 10.1387C51.2512 10.1119 50.1066 10.3947 49.0941 10.958C48.0816 11.5212 47.2379 12.3445 46.6501 13.3427V0.172852H42.293V29.4688H46.6501C46.6501 29.1721 46.6501 18.7816 46.6501 18.7816C46.6501 15.6946 47.8619 13.4352 50.8084 13.4352C54.6046 13.4352 54.6209 17.4178 54.6209 19.6962C54.6209 22.9165 54.6209 25.5189 54.6209 28.7393V29.4987H59.0271C59.0271 29.3708 59.0488 29.2728 59.0488 29.1721C59.0488 25.5108 59.0951 21.8522 59.0325 18.1909C58.9916 15.6538 58.5015 10.1387 52.4097 10.1387Z" fill="currentColor"></path>
      ...
      <nav id='nav'>
        <div className="hidden md:flex gap-6 md:gap-10 items-center">
          {["Services", "Our Work", "About Us", "InSight", "Contact"].map((item, index) => (
            <a
              key={index}
              onClick={() => scrollToSection(item.toLowerCase().replace(' ', '-'))}
              className={`text-sm sm:text-md md:text-lg capitalize font-light ${activeSection === item.toLowerCase().replace(' ', '-') ? 'text-yellow-500' : ''} ${item === "Contact" ? "ml-8 md:ml-32" : ""}`}
            >
              {item}
            </a>
          ))}
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="fixed inset-0 font-['Founder_Grotesk_X-Condensed'] bg-gray-900 bg-opacity-95 flex flex-col items-center justify-center z-50 transition-transform ease-in-out duration-500">
          <button
            className="absolute top-4 px-8 right-4 text-white text-2xl"
            onClick={toggleMenu}
          >
            {/* Close Icon */}
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          {["Services", "Our Work", "About Us", "InSight", "Contact"].map((item, index) => (
            <a
              ...
              className={`text-white text-lg capitalize font-light py-7 ${activeSection === item.toLowerCase().replace(' ', '-') ? 'text-yellow-500' : ''}`}
            >
              {item}
            </a>
          ))}
